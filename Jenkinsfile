    ...
    stages
    {
        ...
        stage("Trigger CD")
        {
            steps
            {
                script
                {
                    sh "curl -vk --user jenkins:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://3.106.192.148:8080/job/github-complete-pipeline/buildWithParameters?token=github'"
                }
            }
        }
    }
